...
const { createPPF, createPPL, createPPD } = require("../wallet_transaction/index")
...
const createMember = async (req, res, next) => {
    try {
        ...
        if ([username, password, email, user_id].includes('')) {
        ...
        let check_email = await User.find({ email }).explain('executionStats');
        ...
        let profileDetails = {
            ...
            vip_level: data.data.vip_level,
            kyc_is_activated: false,
            phone: data.data.phoneNumber ? data.data.phoneNumber : "-" ,
            next_level_point: 1,
            total_wagered: 0,
        ...
        const createUsdt = (async(user_id, bal)=>{
            let balance =  bal
        ...
        if (!existingUser) {
            ...
            createPPL(user_id)
            createPPD(user_id)
            createUsdt(user_id, data.data.usdt_balance)
